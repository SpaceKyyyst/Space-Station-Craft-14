...
public final class AtmosCell {
    ...
    private void updateReactionFlag() {
        ...
        for (int i = 0; i < GasType.COUNT; i++) {
            if (moles[i] > 0.01f) {
                ...
                if (g != null) { if (g.isFlammable()) flammable = true; if (g.isOxidizer()) oxidizer = true; }
            }
        }
        hasReactions = flammable && oxidizer && temperatureK >= 373f && temperatureK <= 1643f;
    }
    public boolean canReact() { return hasReactions && getTotalMoles() > 0.1f; }
    ...
    public float diffuseTo(AtmosCell target, float permeability, float deltaTime) {
        ...
        // ✅ Убран порог 0.5f. Диффузия идёт от любого перепада.
        if (Math.abs(deltaP) < 1e-6f) return 0f;
        
		// ✅ Стало (в 2.5× быстрее):
		float rate = permeability * 0.5f * deltaTime;
        ...
        if (transferred > 0.001f) {
            ...
            this.setTemperature(newT); target.setTemperature(newT);
        }
    ...
    public void readFromNBT(net.minecraft.nbt.CompoundTag tag) {
        ...
        if (tag.contains("temp")) temperatureK = tag.getFloat("temp").orElse(294.15f);
        dirty = true; updateReactionFlag();
    }
}
