...
public record DecalData(BlockPos pos, Direction face, String decalId, int rotation) {
    
    public static final Codec<DecalData> CODEC = RecordCodecBuilder.create(instance -> instance.group(
    ...
    public static final StreamCodec<RegistryFriendlyByteBuf, DecalData> STREAM_CODEC = new StreamCodec<>() {
        @Override
        public DecalData decode(RegistryFriendlyByteBuf buffer) {
            return new DecalData(buffer.readBlockPos(), buffer.readEnum(Direction.class), buffer.readUtf(), buffer.readVarInt());
        }
        @Override
