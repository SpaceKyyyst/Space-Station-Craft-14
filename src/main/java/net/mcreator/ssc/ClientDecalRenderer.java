...
public class ClientDecalRenderer {
    ...
    public static void ensureRegistered() {
        ...
        NeoForge.EVENT_BUS.addListener(RenderLevelStageEvent.AfterTranslucentBlocks.class,
                ClientDecalRenderer::renderDecals);

        NeoForge.EVENT_BUS.addListener(ClientPlayerNetworkEvent.LoggingOut.class,
                e -> DecalNetwork.CLIENT_DECAL_STORAGE.clear());
    }
    ...
    private static void renderDecals(RenderLevelStageEvent event) {
        ...
        for (var entry : DecalNetwork.CLIENT_DECAL_STORAGE.entrySet()) {
            ...
            for (DecalData decal : decals) {
                ResourceLocation texture = ResourceLocation.fromNamespaceAndPath("ssc_14",
                        "textures/" + decal.decalId() + ".png");
