    ...
    public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
        ...
        if (Ssc14ModItems.FLOUR.get() == (entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem()) {
            if (world instanceof Level _level) {
                if (!_level.isClientSide()) { // ИСПРАВЛЕНО
                    var sound = BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.fromNamespaceAndPath("ssc_14", "package_opening")).orElse(null);
                    if (sound != null) _level.playSound(null, BlockPos.containing(x, y, z), sound, SoundSource.MASTER, 1, (float) Mth.nextDouble(RandomSource.create(), 0.9, 1.1));
                }
            }
            itemstack.shrink(1);
        ...
        // ===== МОЛОКО =====
            ...
            if (world instanceof Level _level) {
                if (!_level.isClientSide()) { // ИСПРАВЛЕНО
                    var sound = BuiltInRegistries.SOUND_EVENT.get(ResourceLocation.fromNamespaceAndPath("ssc_14", "items_bottle_open")).orElse(null);
                    if (sound != null) _level.playSound(null, BlockPos.containing(x, y, z), sound, SoundSource.MASTER, 1, (float) Mth.nextDouble(RandomSource.create(), 0.9, 1.1));
                }
            }
            itemstack.shrink(1);
