		...
		if (Ssc14ModVariables.MapVariables.get(world).station_code < 6 && Ssc14ModVariables.MapVariables.get(world).station_code != 4) {
			...
			Component announcementMessage = Component
					.literal("Внимание " + Ssc14ModVariables.MapVariables.get(world).station_name + "! Код КРАСНЫЙ!" + "\n"
							+ "Существует прямая угроза работе станции. Служба безопасности имеет право носить оружие наготове. Рекомендуются обыски персонала и отсеков. Персонал станции обязан повиноваться законным требованиям службы безопасности.")
			...
			var soundEvent = net.minecraft.core.registries.BuiltInRegistries.SOUND_EVENT.getValue(ResourceLocation.parse("ssc_14:red_code"));
			if (soundEvent != null) {
