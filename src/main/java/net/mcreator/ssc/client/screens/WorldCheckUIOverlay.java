...
public class WorldCheckUIOverlay {
	...
	public static void eventHandler(RenderGuiEvent.Pre event) {
		...
		if (WorldObjectCheckUIProcedure.execute(world, entity)) {
			...
			event.getGuiGraphics().drawString(Minecraft.getInstance().font, Component.translatable("gui.ssc_14.world_check_ui.label_proc_wo_ctext_p_2"), w / 2 + 5, h / 2 + 14, -1, false);
			event.getGuiGraphics().drawString(Minecraft.getInstance().font,

					WOCtextP2Procedure.execute(world, entity), w / 2 + 5, h / 2 + 26, -16726017, false);
		}
	}
}